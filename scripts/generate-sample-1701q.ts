/**
 * Generate a sample 1701Q overlay PDF for visual verification.
 * Mirrors scripts/generate-sample-1701a.ts.
 *
 * Usage:
 *   pnpm tsx scripts/generate-sample-1701q.ts
 *
 * Writes:
 *   tmp/1701Q-sample-overlay.pdf
 */
import { writeFileSync } from "node:fs";
import { renderForm1701QOverlay, buildForm1701QValues } from "../lib/pdf/bir/1701Q";
...
const sampleData: FilingPdfData = {
  ...
  overpayment: null,
  allReturns: [
    // Prior 1701Q (Q1) — its netTaxDue feeds into Item 56 (Prior Q Payments).
    // The 8% tax on Q1's cumulative (₱39,497.80 - 250K = 0; no 250K income
    // exceeds 0; tax due is 0 because cumulative is below the 250K
    // threshold for Q1). For the sample we use a Q1 netTaxDue of 0.
    { formType: "FORM_1701Q", quarter: 1, netTaxDue: new Decimal("0") },
  ],
};

async function main() {
  console.log("Built value map keys:", Object.keys(buildForm1701QValues(sampleData)).length);

  const result = await renderForm1701QOverlay(sampleData);
  const out = join(process.cwd(), "tmp", "1701Q-sample-overlay.pdf");
